/** Shared globe geometry: fixed point positions and reveal order.
 *  Vendored from web/src/lib/sphereLayout.ts (identical algorithm + constants)
 *  so a DB row's point_index maps to the same position on the slide's globe. */
...
export function getSphereLayout(): SphereLayout {
